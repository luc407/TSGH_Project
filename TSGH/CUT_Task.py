...
# =============================================================================
    def FeatureExtraction(self):
        def GetTrialCmd(i,eyePosition,ACT_Trial,act_time,CmdTime):
            duration = int(4*24) # respond period = seconds*fps            
            ...
            if trg_ind.any(): 
                ...
                ACT_Trial.append(np.nanpercentile(eyePosition[:,trg_ind:trg_ind+duration],50,axis = 1))
            else:
                ACT_Trial.append(np.nanpercentile(eyePosition[:,CmdTmp],50,axis = 1)) 
            i = CmdTmp[end_ind]+1
            return ACT_Trial, i
        ...
        for i in range(0,len(nb.CUT_TIME)):
            ...
            if delete.any():
                self.CmdTime[nb.CUT_TIME[i]] = np.delete(temp, delete)
        ...
        while(i < len(OD[0,:])):
            if i in self.CmdTime['O_t']:
                ...
                if rd > 0 and rd < len(O_trg_ind):  # 2~(total session - 1)
                    ...
                    continue
                OD_ACT.append(np.array(stats.mode(OD[:,tmp],axis = 1,nan_policy='omit')[0].reshape(-1), dtype=float))
                OS_ACT.append(np.array(stats.mode(OS[:,tmp],axis = 1,nan_policy='omit')[0].reshape(-1), dtype=float))
...
# =============================================================================
    ...
    def GetDiagnosis(self):
        ...
        nb.CUT_Save._CUT_dx['Examine Date'].append(self.FolderName.split('_')[0])
