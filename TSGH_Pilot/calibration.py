...
class EyeTrackSystem:
    def __init__(self, master, csv_path):
        ...
        height = im.shape[0]
        width = im.shape[1]
        self.framecnt = int(cv2.VideoCapture.get(self.cap, int(cv2.CAP_PROP_FRAME_COUNT)))
...
# =============================================================================
            ...
            if len(self.xy)>=2*self.framecnt:
                self.done_button = tk.Button(self.master, text="Done!", command=self.done)
                self.done_button.pack(side=tk.LEFT, anchor=tk.S, pady=50)
            else:
                try: self.done_button.destroy()
            ...
            if len(self.xy)%2 == 0: self.textvar.set_text("Select center of OD pupil."); self.fig.canvas.draw()
            elif len(self.xy)%2 == 1: self.textvar.set_text("Select center of OS pupil."); self.fig.canvas.draw()
        else:
            print ('Clicked ouside axes bounds but inside plot window')
    ...
    def done(self):
        even = [i for i in range(0,self.framecnt) if i%2==0]
        odd = [i for i in range(0,self.framecnt) if i%2==0]
        self.OD = self.xy[even,:]
        self.OS = self.xy[odd,:]
        self.master.destroy()
...
# =============================================================================
    ...
    def startCalibrate(self):
        ...
        self.popBack_button = tk.Button(self.master, text="Back", command=self.preFrame)
        self.popBack_button.pack(side=tk.LEFT, padx=400, anchor=tk.S, pady=50) 
